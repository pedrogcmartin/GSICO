...
def convertRGB2YUV(gaussians, output_path):
    # Extract RGB SH coefficients
    ...
    f_dc_yuv = rgb2yuv(f_dc)  # (N, 3)

    # Convert SH coefficients for each coefficient across all Gaussians
    f_rest_yuv = np.zeros_like(f_rest)
    for i in range(15):
        f_rest_yuv[:, i, :] = rgb2yuv(f_rest[:, i, :])

    # Flatten back to (N, 45)
    f_rest_yuv = f_rest_yuv.reshape(-1, 45, order='F')

    # Update gaussians with new YUV values
    gaussians[:, 6:9] = f_dc_yuv
    gaussians[:, 9:54] = f_rest_yuv
...
if __name__ == "__main__":
    ...
    convertRGB2YUV(gaussians, args.output_path)
