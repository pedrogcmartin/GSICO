...
def main():
    ...
    safe_move(scene_dir / "gaussian_model_3dgs.py", scene_dir / "gaussian_model.py")

    # 1) JPEG XL image decoding
    print("Step 1: JPEG XL image decoding\n")
    run(["bash", "jxl_decoder_3dgs.sh", args.dataset, args.scene, args.exp])

    # 2) Gaussian creation
    print("\nStep 2: Reading Gaussians from the decoded images\n")
    run(["python", "-m", "compression.decompress_3dgs", "--compressed_model_path", str(decoded_root)], env=gpu_env)
    
    # 3) YUV to RGB conversion
    print("\nStep 3: Convert SH from YUV to RGB\n")
    decompressed_model = decoded_root / "decompressed_model"
    iter1 = decompressed_model / "point_cloud" / "iteration_1"
    ply_in = iter1 / "point_cloud.ply"
    run(["python", "yuv2rgb.py", str(ply_in), str(iter1)])
    yuv_dir = iter1 / "yuv"
    ensure_dir(yuv_dir)
    safe_move(ply_in, yuv_dir / "point_cloud.ply")
    safe_copy(iter1 / "rgb" / "point_cloud.ply", iter1 / "point_cloud.ply")

    safe_move(scene_dir / "gaussian_model.py", scene_dir / "gaussian_model_3dgs.py")
