...
import time
from compression.png_3dgs import PNGCodec_3dgs
...
def save_gaussian_images(images, file_path, compression_config):
    ...
    for attribute in experiment['attributes']:
        ...
        if attr_name == '_features_rest':
            attr_np = images[:, :, 9:54][:, :, [i + 15 * j for i in range(15) for j in range(3)]]

        if attr_name == '_scaling':
            attr_np = images[:, :, 62-4-3:62-4]

        if attr_name == '_rotation':
            attr_np = images[:, :, 62-4:]

        if attr_name == '_opacity':
            attr_np = images[:, :, 62-4-3-1:62-4-3]
...
def prune_gaussians(gaussians, n_gaussians_prune):
    print(f"Pruning {n_gaussians_prune} Gaussians with lowest opacity...")
    sorted_indices = np.argsort(gaussians[:, 54])
    keep_indices = sorted_indices[n_gaussians_prune:]
...
def create_clusters(gaussians, num_clusters=1024):
    ...
    #features = gaussians[:, 9:54]
    features = np.concatenate([gaussians[:, :3], gaussians[:, 6:24]], axis=1)
...
def mean_distance(candidate, assigned_neighbors, gaussians):
    ...
    if not assigned_neighbors:
        ...
        return np.sum((gaussians[assigned_neighbors, 9:24] - candidate[9:24])**2, axis=1).mean()
...
def sort_blocks(gaussians, blocks, block_size, num_blocks, w_grid_size, h_grid_size, file_path, compression_config, exp):
    ...
    centroids = np.array([np.concatenate([block[:, :3], block[:, 6:24]], axis=1).mean(axis=0) for block in blocks])
    ...
    for block_id in tqdm(range(num_blocks), desc="Sorting Gaussians within blocks"):
        ...
        # Pick first Gaussian
        first_idx = np.argmin(np.linalg.norm(np.concatenate([block[:, :3], block[:, 6:24]], axis=1) - np.median(np.concatenate([block[:, :3], block[:, 6:24]], axis=1), axis=0), axis=1))
