...
def compress_attr(attr_config, gaussians, out_folder):
    ...
    if attr_config.get('normalize', False):
        ...
        min_val, max_val = codec.encode_with_normalization(attr_np, attr_config, attr_name, out_file, **attr_params)
        return file_name, min_val, max_val
    else:
        codec.encode(attr_np, attr_config, out_file, **attr_params)
...
def decompress_attr(gaussians, attr_config, compressed_file, min_val, max_val):
    ...
    if attr_config.get('normalize', False):
        print(attr_name, "was normalized.")
        decompressed_attr = codec.decode_with_normalization(compressed_file, attr_config, min_val, max_val)
    else:
        decompressed_attr = codec.decode(compressed_file, attr_config)
