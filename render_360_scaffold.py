...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    render_path = os.path.join(model_path, name, "renders360")
...
def render_sets(edp : int, nframes : int, dataset : ModelParams, iteration : int, pipeline : PipelineParams):
    with torch.no_grad():
        ...
        render_set(dataset.model_path, "renders360", scene.loaded_iter, getRender360Cameras(edp, scene.getTrainCameras(), n_frames=nframes), gaussians, pipeline, background)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-edp", type=int, default=0)
