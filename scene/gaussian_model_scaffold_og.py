...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, disable_xyz_log_activation):
        ...
        self._features_offset = torch.empty(0)
        self._features_anchor = torch.empty(0)
    ...
    def capture(self):
        return (
            ...
            self._xyz,
            self._features_offset,
            self._features_anchor,
            self._features_dc,
            self._features_rest,
            self._scaling,
            self._rotation,
    ...
    @property
    def get_features_offset(self):
        return self._features_offset

    @property
    def get_features_anchor(self):
        return self._features_anchor
    @property
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(self._features_offset.shape[1]):
            l.append('f_offset_{}'.format(i))
        for i in range(self._features_anchor.shape[1]):
    ...
    def save_ply(self, path):
        ...
        f_offset = self._features_offset.detach().cpu().numpy()
        f_anchor = self._features_anchor.detach().cpu().numpy()
        ...
        attributes = np.concatenate((xyz, normals, f_offset, f_anchor, opacities, scale, rotation), axis=1)
