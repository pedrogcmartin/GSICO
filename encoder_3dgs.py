...
def main():
    ...
    print("Step 1: Convert SH from RGB to YUV\n")
    run(["python", "rgb2yuv.py", str(pc_ply), str(pc_dir)])
    ensure_dir(pc_dir / "rgb")
    safe_move(pc_ply, pc_dir / "rgb" / "point_cloud.ply")
    safe_move(pc_dir / "yuv" / "point_cloud.ply", pc_ply)

    # 2) BB sorting
    print("Step 2: BB image sorting\n")
    run(["python", "bb_sorting_3dgs.py", str(args.exp), str(pc_ply), str(comp_root), str(cfg_root / "compression" / "generate_images_3dgs.yaml"), str(args.block_size)], env=gpu_env)
    
    # 3) JPEG XL image coding
    print("Step 3: JPEG XL image coding\n")
    ensure_dir(bitstream_root)
    run(["bash", "jxl_encoder_3dgs.sh", args.dataset, args.scene, args.exp, str(args.q), str(args.e)])
